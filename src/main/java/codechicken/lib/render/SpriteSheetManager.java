...
public class SpriteSheetManager {
    ...
    public static class SpriteSheet implements IIconSelfRegister {
        ...
        public void requestIndicies(int... indicies) {
            for (int i : indicies) setupSprite(i);
        }
        ...
        public void registerIcons(IIconRegister register) {
            ...
            if (TextureUtils.refreshTexture(textureMap, resource.getResourcePath())) {
                ...
                for (TextureSpecial sprite : sprites)
                    if (sprite != null) textureMap.setTextureEntry(sprite.getIconName(), sprite);
            } else {
                for (int i : newSprites) textureMap.setTextureEntry(sprites[i].getIconName(), sprites[i]);
            }
            newSprites.clear();
        }
    ...
    public static SpriteSheet getSheet(int tilesX, int tilesY, ResourceLocation resource) {
        ...
        if (sheet == null) spriteSheets.put(resource.toString(), sheet = new SpriteSheet(tilesX, tilesY, resource));
        return sheet;
    }
}
