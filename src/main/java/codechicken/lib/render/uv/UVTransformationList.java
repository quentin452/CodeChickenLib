...
public class UVTransformationList extends UVTransformation {
    ...
    public UVTransformationList(UVTransformation... transforms) {
        for (UVTransformation t : transforms)
            if (t instanceof UVTransformationList) transformations.addAll(((UVTransformationList) t).transformations);
    ...
    public void apply(UV uv) {
        for (int i = 0; i < transformations.size(); i++) transformations.get(i).apply(uv);
    }
    ...
    public UVTransformationList prepend(UVTransformation t) {
        ...
        if (t instanceof UVTransformationList) transformations.addAll(0, ((UVTransformationList) t).transformations);
    ...
    private void compact() {
        ...
        if (newList.size() < transformations.size()) transformations = newList;
    }
    ...
    public String toString() {
        String s = "";
        for (UVTransformation t : transformations) s += "\n" + t.toString();
        return s.trim();
    }
}
