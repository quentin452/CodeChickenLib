...
public class QBImporter {
    ...
    private static class ImagePackNode {
        ...
        public boolean pack(QBImage img) {
            if (child1 != null) return child1.pack(img) || child2.pack(img);
        ...
        private int getFit(int w, int h) {
            if (w == rect.w && h == rect.h) return 2;
            if (w == rect.h && h == rect.w) return 3;
            if (rect.w >= w && rect.h >= h) return 4;
            if (rect.w >= h && rect.h >= w) return 5;
        ...
        public static ImagePackNode pack(List<QBImage> images, boolean square) {
            ...
            while (node.rect.area() < area) nextSize(node.rect, square);
    ...
    private static class ImageTransform {
        ...
        public int access(QBImage img, int u, int v) {
            ...
            if (flipU()) u = img.width() - 1 - u;
            if (flipV()) v = img.height() - 1 - v;
        ...
        public UV transform(UV uv) {
            ...
            if (flipU()) uv.u = 1 - uv.u;
            if (flipV()) uv.v = 1 - uv.v;
    ...
    public static class QBImage implements Comparable<QBImage> {
        ...
        public boolean equals(QBImage img, ImageTransform t) {
            for (int u = 0; u < img.width(); u++)
                for (int v = 0; v < img.height(); v++) if (t.access(this, u, v) != img.data[u][v]) return false;
    ...
    public static class QBQuad {
        ...
        public static QBQuad restore(Rectangle4i flat, int side, double d, QBImage img) {
            ...
            for (Vertex5 vert : quad.verts) vert.apply(t);
    ...
    public static class QBCuboid {
        ...
        public void extractQuads(List<QBQuad> quads) {
            ...
            for (int s = 0; s < 6; s++) if ((sides & 1 << s) == 0) quads.add(extractQuad(s, box));
        }
    ...
    public static class QBMatrix {
        ...
        public void readMatrix(DataInputStream din, boolean compressed) throws IOException {
            if (compressed) {
                ...
                while (z < size.z) {
                    ...
                    while (true) {
                        ...
                        if (data == CODEFLAG) {
                            ...
                            for (int j = 0; j < count; j++, index++) matrix[index % size.x][index / size.x][z] = data;
                        } else {
                ...
                for (int z = 0; z < size.z; z++)
                    for (int y = 0; y < size.y; y++) for (int x = 0; x < size.x; x++) matrix[x][y][z] = din.readInt();
            }
        }
        ...
        private boolean voxelFull(boolean[][][] solid, CuboidCoord c) {
            for (BlockCoord b : c) if (matrix[b.x][b.y][b.z] == 0) return false;
            for (BlockCoord b : c) solid[b.x][b.y][b.z] = false;
            return true;
        }
        ...
        private QBCuboid expand(boolean[][][] solid, BlockCoord b) {
            ...
            for (int s = 0; s < 6; s++) {
                ...
                while (slice.getSide(s) >= 0 && slice.getSide(s) < size.getSide(s)) {
                    if (!voxelFull(solid, slice)) break;
                    slice.expand(s ^ 1, -1);
                    slice.expand(s, 1);
                    c.expand(s, 1);
                }
        ...
        public List<QBQuad> extractQuads(boolean texturePlanes) {
            ...
            for (QBCuboid c : rectangulate()) c.extractQuads(quads);

            if (texturePlanes) optimisePlanes(quads);
        ...
        private void optimisePlanes(List<QBQuad> quads) {
            ...
            for (QBQuad quad : quads) map.put(quad.side | ((int) quad.verts[0].vec.getSide(quad.side)) << 3, quad);
            ...
            for (Integer key : map.keySet()) {
                ...
                for (QBQuad q : plane) {
                    ...
                    for (int u = 0; u < from.width(); u++)
                        for (int v = 0; v < from.height(); v++) img.data[du + u][dv + v] = from.data[u][v];
                }
        ...
        public CCModel buildModel(List<QBQuad> quads, BufferedImage img, boolean scaleMC) {
            ...
            if (scaleMC) m.apply(new Scale(1 / 16D));
            m.computeNormals();
            return m;
        }
    ...
    public static class RasterisedModel {
        ...
        public void export(File objFile, File imgDir) {
            try {
                ...
                for (Map.Entry<String, Holder> e : map.entrySet()) modelMap.put(e.getKey(), e.getValue().m);
    ...
    public static QBModel loadQB(InputStream input) throws IOException {
        ...
        if (visEncoded) throw new IllegalArgumentException("Encoded Visiblity States not supported");
        ...
        for (int i = 0; i < m.matrices.length; i++) {
            ...
            if (colorFormat == 1) mat.convertBGRAtoRGBA();
        }
