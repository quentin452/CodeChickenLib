...
public class ModularASMTransformer {
    ...
    public static class ClassNodeTransformerList {
        ...
        public byte[] transform(byte[] bytes) {
            ...
            if (!methodsToSort.isEmpty()) cv = new LocalVariablesSorterVisitor(methodsToSort, cv);
    ...
    public abstract static class MethodTransformer extends ClassNodeTransformer {
        ...
        public void transform(ClassNode cnode) {
            ...
            if (mv == null) throw new RuntimeException("Method not found: " + method);
    ...
    public static class MethodWriter extends ClassNodeTransformer {
        ...
        public void transform(ClassNode cnode) {
            MethodNode mv = findMethod(method, cnode);
            if (mv == null) mv = (MethodNode) method.visitMethod(cnode, access, exceptions);
            else {
                mv.access = access;
                mv.instructions.clear();
                if (mv.localVariables != null) mv.localVariables.clear();
                if (mv.tryCatchBlocks != null) mv.tryCatchBlocks.clear();
            }
    ...
    public static class MethodInjector extends MethodTransformer {
        ...
        public void transform(MethodNode mv) {
            if (needle == null) {
                ...
                if (before) mv.instructions.insert(injection.rawListCopy());
                ...
                for (InsnListSection key : InsnComparator.findN(mv.instructions, needle.list)) {
                    ...
                    else key.insert(injectBlock.list.list);
                }
            }
        }
    }
