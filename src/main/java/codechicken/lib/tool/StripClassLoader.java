...
public class StripClassLoader extends URLClassLoader {
    ...
    public Class<?> loadClass(String name) throws ClassNotFoundException {
        if (!name.startsWith("codechicken.lib")) return super.loadClass(name);

        try {
            String resName = name.replace('.', '/') + ".class";
            InputStream res = getResourceAsStream(resName);
            if (res == null) throw new ClassNotFoundException("Could not find resource: " + resName);
    ...
    public static byte[] readFully(InputStream is) throws IOException {
        ...
        while ((read = is.read(data, 0, data.length)) > 0) buffer.write(data, 0, read);
