...
public class WorldExtensionManager {
    ...
    public static class WorldExtensionEventHandler {
        ...
        public void onChunkLoad(ChunkEvent.Load event) {
            ...
            for (WorldExtension extension : worldMap.get(event.world)) extension.loadChunk(event.getChunk());
        }
        ...
        public void onWorldSave(WorldEvent.Save event) {
            if (worldMap.containsKey(event.world))
                for (WorldExtension extension : worldMap.get(event.world)) extension.save();
        }
    ...
    private static void onWorldLoad(World world) {
        ...
        for (WorldExtension extension : extensions) extension.load();
    }
    ...
    private static void removeChunk(World world, Chunk chunk) {
        for (WorldExtension extension : worldMap.get(world)) extension.remChunk(chunk);
    }

    private static void preTick(World world) {
        for (WorldExtension extension : worldMap.get(world)) extension.preTick();
    }

    private static void postTick(World world) {
        for (WorldExtension extension : worldMap.get(world)) extension.postTick();
    }
