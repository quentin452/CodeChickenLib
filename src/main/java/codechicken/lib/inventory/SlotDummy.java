...
public class SlotDummy extends SlotHandleClicks {
    ...
    public void slotClick(ItemStack held, int button, boolean shift) {
        ...
        if (held != null && (tstack == null || !InventoryUtils.canStack(held, tstack))) {
            ...
            if (shift) quantity = Math.min(stackLimit, held.getMaxStackSize() * 16);
            if (button == 1) quantity = 1;
            ...
            if (quantity <= 0) putStack(null);
            else putStack(InventoryUtils.copyStack(tstack, quantity));
        }
    }

    @Override
    public void putStack(ItemStack stack) {
        if (stack != null && stack.stackSize > stackLimit) stack = InventoryUtils.copyStack(stack, stackLimit);
