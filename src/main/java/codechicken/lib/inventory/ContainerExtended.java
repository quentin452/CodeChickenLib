...
public abstract class ContainerExtended extends Container implements ICrafting {
    ...
    static {
        PacketCustom.assignHandler(netChannel, new IClientPacketHandler() {
            ...
            public void handlePacket(PacketCustom packet, Minecraft mc, INetHandlerPlayClient handler) {
                ...
                if (!(cont instanceof ContainerExtended)) return;

                ContainerExtended c = (ContainerExtended) cont;
                if (packet.getType() == 1) c.netID = packet.readInt();
                else if (c.netID == packet.readInt()) c.handleClientPacket(packet);
            }
        });
        PacketCustom.assignHandler(netChannel, new IServerPacketHandler() {
            ...
            public void handlePacket(PacketCustom packet, EntityPlayerMP sender, INetHandlerPlayServer handler) {
                ...
                ContainerExtended c = (ContainerExtended) cont;
                if (c.netID == packet.readInt()) c.handleServerPacket(packet);
            }
        });
    }
    ...
    public ContainerExtended() {
        crafters.add(this);
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER) netID = ++nextNetworkID;
    }
    ...
    private void sendNetID(EntityPlayerMP player) {
        if (netID == 0) LogManager.getLogger("CodeChickenLib").error("Player added to container with 0 network ID");
        else new PacketCustom(netChannel, 1).writeInt(netID).sendToPlayer(player);
    }

    @Override
    public void removeCraftingFromCrafters(ICrafting icrafting) {
        if (icrafting instanceof EntityPlayerMP) playerCrafters.remove(icrafting);
    ...
    public void sendContainerAndContentsToPlayer(Container container, List<ItemStack> list,
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            } else largeStacks.add(null);
        }

        for (EntityPlayerMP player : playerCrafters) player.sendContainerAndContentsToPlayer(container, list);

        for (int i = 0; i < largeStacks.size(); i++) {
            ItemStack stack = largeStacks.get(i);
            if (stack != null) sendLargeStack(stack, i, playerCrafters);
        }
    }
    ...
    public void sendProgressBarUpdate(Container container, int i, int j) {
        for (EntityPlayerMP player : playerCrafters) player.sendProgressBarUpdate(container, i, j);
    }

    @Override
    public void sendSlotContents(Container container, int slot, ItemStack stack) {
        if (stack != null && stack.stackSize > Byte.MAX_VALUE) sendLargeStack(stack, slot, playerCrafters);
        else for (EntityPlayerMP player : playerCrafters) player.sendSlotContents(container, slot, stack);
    }
    ...
    public ItemStack slotClick(int par1, int par2, int par3, EntityPlayer player) {
        if (par1 >= 0 && par1 < inventorySlots.size()) {
            ...
            if (slot instanceof SlotHandleClicks) return ((SlotHandleClicks) slot).slotClick(this, player, par2, par3);
        }
    ...
    public ItemStack transferStackInSlot(EntityPlayer par1EntityPlayer, int slotIndex) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (!doMergeStackAreas(slotIndex, stack)) return null;
    ...
    public void sendContainerPacket(PacketCustom packet) {
        for (EntityPlayerMP player : playerCrafters) packet.sendToPlayer(player);
    }
    ...
    public void sendProgressBarUpdate(int barID, int value) {
        for (ICrafting crafting : (List<ICrafting>) crafters) crafting.sendProgressBarUpdate(this, barID, value);
    }
}
